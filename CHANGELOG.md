...
- `PlugHandler` and `PlugEnergyMonitoringHandler`: added `get_next_event` returning the next event the device is scheduled to act on (typically the soonest-firing schedule rule), or `None` if no rule is armed. Surfaces the device-computed firing time for sunrise / sunset rules.
